namespace ET
{
    ...
    public static class UnitEntitySystem
    {
        ...
        private static void UpdateRotate(this Unit self, int delta)
        {
            if (!self.m_bRotating)
                return;
            if (self.m_nRemainAngle != 0)
            {
                self.m_nOnFrameRotateAngle = delta * self.m_nRotateSpeed / 1000;
