...
namespace ET
{
    ...
    public class UIMediatorManagerComponentDestroySystem : DestroySystem<UIMediatorManager>
    {
        ...
        {
            self.m_allUiMediator.Clear();
            self.m_allUiMediator = null;
    ...
    public static class UIMediatorManagerComponentSystem
    {
        public static void Init(this UIMediatorManager self, UI ui)
        {
            try
            {
                var mediator = self.m_allUiMediator[ui.Name];
        ...
        public static void Destroy(this UIMediatorManager self, string uiType)
        {
            try
            {
                self.m_allUiMediator[uiType].OnDestroy();
            }
            catch (Exception e)
            {
        ...
        public static void Open(this UIMediatorManager self, string uiType)
        {
            try
            {
                self.m_allUiMediator[uiType].OnOpen();
            }
            catch (Exception e)
            {
        ...
        public static void Close(this UIMediatorManager self, string uiType)
        {
            try
            {
                self.m_allUiMediator[uiType].OnClose();
            }
            catch (Exception e)
            {
                throw new Exception($"UIMediatorManagerComponentSystem.Open: {uiType}Mediator error", e);
        ...
        public static void BeCover(this UIMediatorManager self, string uiType)
        {
            try
            {
                self.m_allUiMediator[uiType].OnBeCover();
            }
            catch (Exception e)
            {
        ...
        public static void UnCover(this UIMediatorManager self, string uiType)
        {
            try
            {
                self.m_allUiMediator[uiType].OnUnCover();
            }
            catch (Exception e)
            {
        ...
        {
            self.m_allUiMediator = new Dictionary<string, IMediator>();
            ...
            foreach (Type type in mediatorAttributes)
            {
                ...
                self.m_allUiMediator.Add(uiTag.Name, uiMediator);
            }
            
            self.m_allUiEvents = new Dictionary<string, IUIEvent>();
            var uiEventTags = Game.EventSystem.GetTypes(typeof(UIEventTagAttribute));
            foreach (Type type in uiEventTags)
            {
                var attrs = type.GetCustomAttributes(typeof(UIEventTagAttribute), false);
                if (attrs.Length <= 0)
                    continue;
                var uiEventTag = attrs[0] as UIEventTagAttribute;
                if (uiEventTag == null)
                    continue;
