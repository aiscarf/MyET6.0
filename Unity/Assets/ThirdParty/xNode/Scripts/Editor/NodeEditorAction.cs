...
namespace XNodeEditor
{
    public partial class NodeEditorWindow
    {
        ...
        public static XNodeEditor.NodeProxy[] copyBuffer = null;
        ...
        private XNodeEditor.NodeProxy hoveredNode = null;
        ...
        public void Controls()
        {
            ...
            switch (e.type)
            {
                ...
                case EventType.MouseDrag:
                    if (e.button == 0)
                    {
                        ...
                        if (currentActivity == NodeActivity.DragNode)
                        {
                            ...
                            {
                                if (Selection.objects[i] is XNodeEditor.NodeProxy)
                                {
                                    XNodeEditor.NodeProxy node = Selection.objects[i] as XNodeEditor.NodeProxy;
                        ...
                        if (IsHoveringPort)
                        {
                            ...
                            else
                            {
                                ...
                                if (hoveredPort.IsConnected)
                                {
                                    ...
                                    draggedOutputTarget = hoveredPort;
                                    var nodeProxy = graphProxy.GetNodeProxy(node);
                                    if (NodeEditor.onUpdateNode != null) NodeEditor.onUpdateNode(nodeProxy);
                                }
                            }
                        }
                    ...
                    if (e.button == 0)
                    {
                        ...
                        if (IsDraggingPort)
                        {
                            ...
                            if (draggedOutputTarget != null && draggedOutput.CanConnectTo(draggedOutputTarget))
                            {
                                ...
                                if (graphProxy.NodeProxies.Count != 0) draggedOutput.Connect(draggedOutputTarget);
                                ...
                                if (connectionIndex != -1)
                                {
                                    ...
                                    var nodeProxy = graphProxy.GetNodeProxy(node);
                                    if (NodeEditor.onUpdateNode != null) NodeEditor.onUpdateNode(nodeProxy);
                                    EditorUtility.SetDirty(graphProxy);
                                }
                            }
                        ...
                        {
                            IEnumerable<XNodeEditor.NodeProxy> nodes = Selection.objects
                                .Where(x => x is XNodeEditor.NodeProxy).Select(x => x as XNodeEditor.NodeProxy);
                            foreach (XNodeEditor.NodeProxy node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        }
                        else if (!IsHoveringNode)
                        {
                ...
                case EventType.KeyDown:
                    ...
                    {
                        if (Selection.objects.Any(x => graphProxy.NodeProxies.Contains(x as XNodeEditor.NodeProxy)))
                        {
                            foreach (var node in graphProxy.NodeProxies)
                            {
                                DeselectNode(node);
                            }
                        }
        ...
        public void Home()
        {
            var nodes = Selection.objects.Where(o => o is XNodeEditor.NodeProxy).Cast<XNodeEditor.NodeProxy>().ToList();
            if (nodes.Count > 0)
            {
        ...
        public void RemoveSelectedNodes()
        {
            ...
            {
                if (item is XNodeEditor.NodeProxy)
                {
                    XNodeEditor.NodeProxy node = item as XNodeEditor.NodeProxy;
        ...
        {
            if (Selection.objects.Length == 1 && Selection.activeObject is XNodeEditor.NodeProxy)
            {
                XNodeEditor.NodeProxy nodeProxy = Selection.activeObject as XNodeEditor.NodeProxy;
                Vector2 size;
                if (nodeSizes.TryGetValue(nodeProxy, out size))
                {
        ...
        public void MoveNodeToTop(XNodeEditor.NodeProxy nodeProxy)
        {
            int index;
            while ((index = graphProxy.NodeProxies.IndexOf(nodeProxy)) != graphProxy.NodeProxies.Count - 1)
            {
                graphProxy.NodeProxies[index] = graphProxy.NodeProxies[index + 1];
                graphProxy.NodeProxies[index + 1] = nodeProxy;
            }
        }
        ...
        public void DuplicateSelectedNodes()
        {
            ...
            XNodeEditor.NodeProxy[] selectedNodes = Selection.objects.Select(x => x as XNodeEditor.NodeProxy)
                .Where(x => x != null && x.graph == graphProxy.graph).ToArray();
        ...
        {
            copyBuffer = Selection.objects.Select(x => x as XNodeEditor.NodeProxy)
                .Where(x => x != null && x.graph == graphProxy.graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(XNodeEditor.NodeProxy[] nodes, Vector2 topLeft)
        {
            ...
            Dictionary<NodeProxy, NodeProxy> substitutes =
                new Dictionary<NodeProxy, NodeProxy>();
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                Type nodeType = srcNode.GetNodeType();
                if (NodeEditorUtilities.GetAttrib(nodeType, out disallowAttrib))
                {
                    int typeCount = graphProxy.NodeProxies.Count(x => x.GetNodeType() == nodeType);
            ...
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                foreach (XNode.NodePort port in srcNode.Ports)
                {
                    for (int c = 0; c < port.ConnectionCount; c++)
                    {
                        ...
                        XNodeEditor.NodeProxy newNodeIn, newNodeOut;
                        if (substitutes.TryGetValue(graphProxy.GetNodeProxy(inputPort.node), out newNodeIn) &&
                            substitutes.TryGetValue(graphProxy.GetNodeProxy(outputPort.node), out newNodeOut))
                        {
        ...
        bool IsHoveringTitle(XNodeEditor.NodeProxy nodeProxy)
        {
            Vector2 mousePos = Event.current.mousePosition;
            //Get node position
            Vector2 nodePos = GridToWindowPosition(nodeProxy.position);
            float width;
            Vector2 size;
            if (nodeSizes.TryGetValue(nodeProxy, out size)) width = size.x;
            else width = 200;
        ...
        public void AutoConnect(XNodeEditor.NodeProxy nodeProxy)
        {
            if (autoConnectOutput == null) return;

            // Find input port of same type
            XNode.NodePort inputPort =
                nodeProxy.Ports.FirstOrDefault(x => x.IsInput && x.ValueType == autoConnectOutput.ValueType);
            // Fallback to input port
            if (inputPort == null) inputPort = nodeProxy.Ports.FirstOrDefault(x => x.IsInput);
