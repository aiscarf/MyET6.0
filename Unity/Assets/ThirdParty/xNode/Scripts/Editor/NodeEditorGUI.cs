...
namespace XNodeEditor
{
    ...
    public partial class NodeEditorWindow
    {
        ...
        private List<NodeProxy> culledNodes;
        ...
        public void DrawConnections()
        {
            ...
            List<RerouteReference> selection = preBoxSelectionReroute != null
            ...
            Color col = GUI.color;
            foreach (NodeProxy nodeProxy in graphProxy.NodeProxies)
            {
                //If a null node is found, return. This can happen if the nodes associated script is deleted. It is currently not possible in Unity to delete a null asset.
                if (nodeProxy == null) continue;

                // Draw full connections and output > reroute
                foreach (XNode.NodePort output in nodeProxy.Outputs)
                {
        ...
        private void DrawNodes()
        {
            ...
            if (Selection.activeObject != null && Selection.activeObject is NodeProxy)
            {
            ...
            if (e.type == EventType.Layout) culledNodes = new List<NodeProxy>();
            for (int n = 0; n < graphProxy.NodeProxies.Count; n++)
            {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graphProxy.NodeProxies[n] == null) continue;
                if (n >= graphProxy.NodeProxies.Count) return;
                NodeProxy nodeProxy = graphProxy.NodeProxies[n];
                // Culling
                if (e.type == EventType.Layout)
                {
                    // Cull unselected nodes outside view
                    if (!Selection.Contains(nodeProxy) && ShouldBeCulled(nodeProxy))
                    {
                        culledNodes.Add(nodeProxy);
                        continue;
                    }
                }
                else if (culledNodes.Contains(nodeProxy)) continue;

                if (e.type == EventType.Repaint)
                {
                    removeEntries.Clear();
                    foreach (var kvp in _portConnectionPoints)
                        if (kvp.Key.node == nodeProxy.node)
                            removeEntries.Add(kvp.Key);
                    foreach (var k in removeEntries) _portConnectionPoints.Remove(k);
                }

                NodeEditor nodeEditor = NodeEditor.GetEditor(nodeProxy, this);
                ...
                //Get node position
                Vector2 nodePos = GridToWindowPositionNoClipped(nodeProxy.position);

                GUILayout.BeginArea(new Rect(nodePos, new Vector2(nodeEditor.GetWidth(), 4000)));

                bool selected = selectionCache.Contains(graphProxy.NodeProxies[n]);
                ...
                if (e.type == EventType.Repaint)
                {
                    ...
                    if (nodeSizes.ContainsKey(nodeProxy)) nodeSizes[nodeProxy] = size;
                    else nodeSizes.Add(nodeProxy, size);

                    foreach (var kvp in NodeEditor.portPositions)
                    {
                        Vector2 portHandlePos = kvp.Value;
                        portHandlePos += nodeProxy.position;
                ...
                if (e.type != EventType.Layout)
                {
                    ...
                    if (windowRect.Contains(mousePos)) hoveredNode = nodeProxy;

                    //If dragging a selection box, add nodes inside to selection
                    if (currentActivity == NodeActivity.DragGrid)
                    {
                        if (windowRect.Overlaps(selectionBox)) preSelection.Add(nodeProxy);
                    }

                    //Check if we are hovering any of this nodes ports
                    //Check input ports
                    foreach (XNode.NodePort input in nodeProxy.Inputs)
                    {
        ...
        private bool ShouldBeCulled(NodeProxy nodeProxy)
        {
            Vector2 nodePos = GridToWindowPositionNoClipped(nodeProxy.position);
            if (nodePos.x / _zoom > position.width) return true; // Right
            else if (nodePos.y / _zoom > position.height) return true; // Bottom
            else if (nodeSizes.ContainsKey(nodeProxy))
            {
                Vector2 size = nodeSizes[nodeProxy];
