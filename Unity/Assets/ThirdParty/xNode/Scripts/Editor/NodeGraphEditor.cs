...
namespace XNodeEditor
{
    ...
    public class NodeGraphEditor : XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor,
        NodeGraphEditor.CustomNodeGraphEditorAttribute, NodeGraphProxy>
    {
        ...
        public virtual void AddContextMenuItems(GenericMenu menu)
        {
            ...
            for (int i = 0; i < nodeTypes.Length; i++)
            {
                ...
                if (type == typeof(XNode.Node)) continue;
                ...
                {
                    int typeCount = target.NodeProxies.Count(x => x.GetNodeType() == type);
                ...
                if (disallowed) menu.AddItem(new GUIContent(path), false, null);
                    ...
                    {
                        var nodeProxy = CreateNode(type, pos);
                        NodeEditorWindow.current.AutoConnect(nodeProxy);
                    });
            }
        ...
        public virtual NodeProxy CreateNode(Type type, Vector2 position)
        {
            Undo.RecordObject(target, "Create Node");
            NodeProxy nodeProxy = target.AddNode(type);
            Undo.RegisterCreatedObjectUndo(nodeProxy, "Create Node");
            ...
            if (!string.IsNullOrEmpty(AssetDatabase.GetAssetPath(target)))
                AssetDatabase.AddObjectToAsset(nodeProxy, target);
            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
            NodeEditorWindow.RepaintAll();
            return nodeProxy;
        }

        public virtual NodeProxy CopyNode(NodeProxy original)
        {
            Undo.RecordObject(target, "Duplicate Node");
            var nodeProxy = target.CopyNode(original);
            Undo.RegisterCreatedObjectUndo(nodeProxy, "Duplicate Node");
            nodeProxy.name = original.name;
            AssetDatabase.AddObjectToAsset(nodeProxy, target);
            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
            return nodeProxy;
        }
        ...
        public virtual bool CanRemove(NodeProxy node)
        {
            // Check graph attributes to see if this node is required
            Type graphType = target.GetGraphType();
            ...
            if (attribs.Any(x => x.Requires(node.GetNodeType())))
            {
                if (target.NodeProxies.Count(x => x.GetNodeType() == node.GetNodeType()) <= 1)
                {
                    return false;
                }
            }
        ...
        public virtual void RemoveNode(NodeProxy nodeProxy)
        {
            if (!CanRemove(nodeProxy)) return;

            // Remove the node
            // Undo.RecordObject(nodeProxy, "Delete Node");
            Undo.RecordObject(target, "Delete Node");
            // foreach (var port in nodeProxy.Ports)
            // foreach (var conn in port.GetConnections())
            // {
            //     var nodeProxy1 = target.GetNodeProxy(conn.node);
            //     Undo.RecordObject(nodeProxy1, "Delete Node");
            // }
            target.RemoveNode(nodeProxy);
            Undo.DestroyObjectImmediate(nodeProxy);
            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
        }
