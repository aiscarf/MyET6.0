...
namespace XNodeEditor.Internal {
	...
	public abstract class NodeEditorBase<T, A, K> where A : Attribute, NodeEditorBase<T, A, K>.INodeEditorAttrib where T : NodeEditorBase<T, A, K> where K : NodeScriptableObject {
			...
			get {
				if (this._objectTree == null) {
					try {
                        ...
                        this._objectTree = PropertyTree.Create(serializedObject);
