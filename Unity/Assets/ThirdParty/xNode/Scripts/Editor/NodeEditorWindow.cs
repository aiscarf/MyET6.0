...
namespace XNodeEditor
{
    ...
    public partial class NodeEditorWindow : EditorWindow
    {
        ...
        public Dictionary<NodeProxy, Vector2> nodeSizes
        {
            get { return _nodeSizes; }
        }

        private Dictionary<NodeProxy, Vector2> _nodeSizes = new Dictionary<NodeProxy, Vector2>();
        public NodeGraphProxy graphProxy { get; set; }
        ...
        {
            NodeGraphProxy nodeGraph = Selection.activeObject as NodeGraphProxy;
        ...
        {
            NodeGraphEditor graphEditor = NodeGraphEditor.GetEditor(graphProxy, this);
        ...
        public void Save()
        {
            if (AssetDatabase.Contains(graphProxy))
            {
        ...
        public void SaveAs()
        {
            ...
            {
                NodeGraphProxy existingGraph = AssetDatabase.LoadAssetAtPath<NodeGraphProxy>(path);
                if (existingGraph != null) AssetDatabase.DeleteAsset(path);
                AssetDatabase.CreateAsset(graphProxy, path);
                EditorUtility.SetDirty(graphProxy);
                if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
            }
        }
        ...
        public void SelectNode(NodeProxy node, bool add)
        {
            if (add)
            {
        ...
        public void DeselectNode(NodeProxy node)
        {
            List<Object> selection = new List<Object>(Selection.objects);
        ...
        {
            NodeGraphProxy nodeGraph = EditorUtility.InstanceIDToObject(instanceID) as NodeGraphProxy;
            if (nodeGraph != null)
            {
                Open(nodeGraph);
                return true;
        ...
        public static NodeEditorWindow Open(NodeGraphProxy graphProxy)
        {
            if (!graphProxy) return null;

            NodeEditorWindow w = GetWindow(typeof(NodeEditorWindow), false, "xNode", true) as NodeEditorWindow;
            w.wantsMouseMove = true;
            w.graphProxy = graphProxy;
            return w;
        }
