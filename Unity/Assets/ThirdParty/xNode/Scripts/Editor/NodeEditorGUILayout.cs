...
namespace XNodeEditor
{
    ...
    public static class NodeEditorGUILayout
    {
        ...
        //     NodeProxy nodeProxy = property.serializedObject.targetObject as NodeProxy;
        //     XNode.NodePort port = nodeProxy.GetPort(property.name);
        //     PropertyField(property, label, port, includeChildren);
        // }
        //
        // /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        // public static void PropertyField(SerializedProperty property, XNode.NodePort port, bool includeChildren = true,
        //     params GUILayoutOption[] options)
        // {
        ...
        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label, NodeGraphProxy graphProxy,
            XNode.NodePort port,
            ...
            else
            {
                ...
                if (port.direction == XNode.NodePort.IO.Input)
                {
                    ...
                    bool usePropertyAttributes = dynamicPortList ||
                                                 ...
                                                 (showBacking == XNode.Node.ShowBackingValue.Unconnected &&
                                                  port.IsConnected);
                    ...
                    if (dynamicPortList)
                    {
                        ...
                        DynamicPortList(graphProxy, property.name, type, property.serializedObject, port.direction,
                            connectionType);
                        return;
                    }
                ...
                else if (port.direction == XNode.NodePort.IO.Output)
                {
                    ...
                    {
                        Type type = GetType(property);
                        XNode.Node.ConnectionType connectionType = outputAttribute != null
                ...
                NodeEditor editor = NodeEditor.GetEditor(graphProxy.GetNodeProxy(port.node), NodeEditorWindow.current);
        ...
        public static void PortField(NodeGraphProxy graphProxy, XNode.NodePort port, params GUILayoutOption[] options)
        {
            PortField(null, graphProxy, port, options);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, NodeGraphProxy graphProxy, XNode.NodePort port,
            params GUILayoutOption[] options)
        {
            if (port == null) return;
            ...
            PortField(graphProxy, position, port);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(NodeGraphProxy graphProxy, Vector2 position, XNode.NodePort port)
        {
            if (port == null) return;
        ...
        public static void AddPortField(NodeGraphProxy graphProxy, XNode.NodePort port)
        {
            if (port == null) return;
            Rect rect = new Rect();
        ...
        public static void PortPair(NodeGraphProxy graphProxy, XNode.NodePort input, XNode.NodePort output)
        {
            GUILayout.BeginHorizontal();
            NodeEditorGUILayout.PortField(graphProxy, input, GUILayout.MinWidth(0));
            NodeEditorGUILayout.PortField(graphProxy, output, GUILayout.MinWidth(0));
        ...
        public static bool IsDynamicPortListPort(NodeGraphProxy graphProxy, XNode.NodePort port)
        {
            string[] parts = port.fieldName.Split(' ');
            if (parts.Length != 2) return false;
            Dictionary<string, ReorderableList> cache;
            if (reorderableListCache.TryGetValue(graphProxy.GetNodeProxy(port.node), out cache))
            {
        ...
        public static void DynamicPortList(NodeGraphProxy graphProxy, string fieldName, Type type,
            SerializedObject serializedObject,
            XNode.NodePort.IO io, XNode.Node.ConnectionType connectionType = XNode.Node.ConnectionType.Multiple,
            XNode.Node.TypeConstraint typeConstraint = XNode.Node.TypeConstraint.None,
            Action<ReorderableList> onCreation = null)
        {
            NodeProxy nodeProxy = serializedObject.targetObject as NodeProxy;

            var indexedPorts = nodeProxy.DynamicPorts.Select(x =>
            {
            ...
            if (list == null)
            {
                ...
                list = CreateReorderableList(graphProxy, fieldName, dynamicPorts, arrayData, type, serializedObject, io,
        ...
        private static ReorderableList CreateReorderableList(NodeGraphProxy graphProxy, string fieldName,
            List<XNode.NodePort> dynamicPorts,
            SerializedProperty arrayData, Type type, SerializedObject serializedObject, XNode.NodePort.IO io,
            XNode.Node.ConnectionType connectionType, XNode.Node.TypeConstraint typeConstraint,
            Action<ReorderableList> onCreation)
        {
            ...
            list.drawElementCallback =
                ...
                {
                    XNode.NodePort port = nodeProxy.GetPort(fieldName + " " + index);
                    ...
                    if (port != null)
                    {
                        ...
                        NodeEditorGUILayout.PortField(graphProxy, pos, port);
                    }
                };
            ...
            list.onReorderCallback =
                (ReorderableList rl) =>
                {
                    ...
                    if (rl.index > reorderableListIndex)
                    {
                        ...
                        {
                            XNode.NodePort port = nodeProxy.GetPort(fieldName + " " + i);
                            XNode.NodePort nextPort = nodeProxy.GetPort(fieldName + " " + (i + 1));
            ...
            list.onAddCallback =
                (ReorderableList rl) =>
                {
                    ...
                    int i = 0;
                    while (nodeProxy.HasPort(newName)) newName = fieldName + " " + (++i);

                    if (io == XNode.NodePort.IO.Output)
                        nodeProxy.AddDynamicOutput(type, connectionType, XNode.Node.TypeConstraint.None, newName);
                    else nodeProxy.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    serializedObject.Update();
                    EditorUtility.SetDirty(nodeProxy);
                    if (hasArrayData)
                    {
            ...
            list.onRemoveCallback =
                (ReorderableList rl) =>
                {
                    ...
                    else
                    {
                        ...
                        nodeProxy.RemoveDynamicPort(dynamicPorts[dynamicPorts.Count() - 1].fieldName);
                        serializedObject.Update();
                        EditorUtility.SetDirty(nodeProxy);
                    }
                    ...
                    if (hasArrayData && arrayData.propertyType != SerializedPropertyType.String)
                    {
                        ...
                        // Error handling. If the following happens too often, file abug report at https://github.com/Siccity/xNode/issues
            ...
            if (hasArrayData)
            {
                ...
                while (dynamicPortCount < arrayData.arraySize)
                {
                    ...
                    int i = 0;
                    while (nodeProxy.HasPort(newName)) newName = arrayData.name + " " + (++i);
                    if (io == XNode.NodePort.IO.Output)
                        nodeProxy.AddDynamicOutput(type, connectionType, typeConstraint, newName);
                    else nodeProxy.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    EditorUtility.SetDirty(nodeProxy);
                    dynamicPortCount++;
                }
