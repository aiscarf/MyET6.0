        ...
        private void OnGUI()
        {
            ...
            if (input == null || input.Trim() == "")
            {
                if (GUILayout.Button("Revert to default") || (e.isKey && e.keyCode == KeyCode.Return))
                {
                    ...
                    NodeEditor.GetEditor((NodeProxy)target, NodeEditorWindow.current).OnRename();
                    AssetDatabase.SetMainObject((target as NodeProxy).graphProxy, AssetDatabase.GetAssetPath(target));
