    ...
    class NodeEditorAssetModProcessor : UnityEditor.AssetModificationProcessor
    {
        ...
        private static AssetDeleteResult OnWillDeleteAsset(string path, RemoveAssetOptions options)
        {
            ...
            for (int i = 0; i < guids.Length; i++)
            {
                ...
                {
                    NodeProxy nodeProxy = objs[k] as NodeProxy;
                    if (nodeProxy.GetNodeType() == scriptType)
                    {
                        if (nodeProxy != null && nodeProxy.graph != null)
                        {
                            // Delete the node and notify the user
                            Debug.LogWarning(
                                nodeProxy.name + " of " + nodeProxy.graph +
                                " depended on deleted script and has been removed automatically.",
                                nodeProxy);
                            nodeProxy.graph.RemoveNode(nodeProxy.node);
                        }
                    }
                }
            }
        ...
        private static void OnReloadEditor()
        {
            ...
            string[] guids = AssetDatabase.FindAssets("t:" + typeof(NodeGraphProxy));
            for (int i = 0; i < guids.Length; i++)
            {
                string assetpath = AssetDatabase.GUIDToAssetPath(guids[i]);
                NodeGraphProxy graphProxy =
                    AssetDatabase.LoadAssetAtPath(assetpath, typeof(NodeGraphProxy)) as NodeGraphProxy;
                graphProxy.NodeProxies.RemoveAll(x => x == null); //Remove null items
                Object[] objs = AssetDatabase.LoadAllAssetRepresentationsAtPath(assetpath);
                ...
                for (int u = 0; u < objs.Length; u++)
                {
                    ...
                    var nodeProxy = objs[u] as NodeProxy;
                    if (!graphProxy.NodeProxies.Contains(nodeProxy))
                        graphProxy.NodeProxies.Add(nodeProxy);
                }
            }
        }
    }
