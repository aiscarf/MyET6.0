...
namespace XNode
{
    ...
    public class NodePort
    {
        ...
        public void InitConnection()
        {
            ...
            {
                connection.node = node.graph.GetNode(connection.nodeId);
            }
        }
        ...
        public void Connect(NodePort port)
        {
            ...
            if (port == null)
            {
                NodeDebug.LogWarning("Cannot connect to null port");
                return;
            }

            if (port == this)
            {
                NodeDebug.LogWarning("Cannot connect port to self.");
                return;
            }

            if (IsConnectedTo(port))
            {
                NodeDebug.LogWarning("Port already connected. ");
                return;
            }

            if (direction == port.direction)
            {
                NodeDebug.LogWarning("Cannot connect two " + (direction == IO.Input ? "input" : "output") +
                                     " connections");
                return;
            }
...
// #endif
        ...
        public bool CanConnectTo(NodePort port)
        {
            ...
            if (input.typeConstraint == XNode.Node.TypeConstraint.Strict && input.ValueType != output.ValueType)
                return false;
            if (input.typeConstraint == XNode.Node.TypeConstraint.InheritedInverse &&
            ...
            if (output.typeConstraint == XNode.Node.TypeConstraint.InheritedInverse &&
                !output.ValueType.IsAssignableFrom(input.ValueType)) return false;
            // Success
            return true;
        }
        ...
        private class PortConnection
        {
            ...
            {
                get { return _port != null ? _port : _port = GetPort(); }
            }
            ...
            // [SerializeField]
            // public List<UnityEngine.Vector2> reroutePoints = new List<UnityEngine.Vector2>();
