...
public class ETCJoystick : ETCBase,IPointerEnterHandler,IDragHandler, IBeginDragHandler, IPointerDownHandler, IPointerUpHandler {
	...
	protected override void Awake (){
		...
		if (joystickType == JoystickType.Dynamic){
			...
			//TODO:王艺棠 
			originalPos = this.rectTransform().anchoredPosition;
			// visible = false;
		}
	...
	public void OnPointerEnter(PointerEventData eventData){
		...
		if (joystickType == JoystickType.Dynamic && !isDynamicActif && _activated && pointId == -1 && dynamicShow){
	...
	private void OnUp(bool real=true){
		...
		if (joystickType == JoystickType.Dynamic){
			...
			//TODO:王艺棠 
			dynamicShow = false;
			transform.rectTransform().anchoredPosition = originalPos;
			isDynamicActif = false;
		}
	...
	private void UpdateJoystick(){
		...
		if (enableKeySimulation && !isOnTouch && _activated && _visible ){
			...
			float x = Input.GetAxisRaw(axisX.unityAxis);
			float y= Input.GetAxisRaw(axisY.unityAxis);
